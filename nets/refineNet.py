...
class refineNet(nn.Module):
    ...
    def _make_layer(self, input_channel, num, output_shape):
        layers = []
        for i in range(num):
            layers.append(Bottleneck(input_channel, int(input_channel/2)))        # 对globalNet的四个深度的featuremap分别接不同深度的bottleneck, feature map大小没变，要保证inchannel=outchannel
        # layers.append(nn.Upsample(size=output_shape, mode='bilinear', align_corners=True))    # 上采样到 64*48
        layers.append(nn.Upsample(size=output_shape, mode='nearest'))
        return nn.Sequential(*layers)

    def _predict(self, input_channel, num_class):   # 对concat后的四组特征接Bottleneck + 3*3Conv + BN
        layers = []
    ...
    def forward(self, x):
        ...
        out = torch.cat(refine_fms, dim=1)        # concat四个尺度的输出
