...
def point_nms(inputs, score=0.1, dis=10):
    ...
    assert len(inputs.shape) == 3
    kept_coors = []
    for c in range(inputs.shape[2]):      # 接收的依然是h*w*c
        heatmap = inputs[...,c]
        x, y = np.where(heatmap > score)    # x是行索引，y是列索引
        coors = list(zip(x, y))
        scores = []
        for coor in coors:
...
def prepare_bbox(bboxs, orih, oriw, outh, outw):
    ...
    alpha = 1.5
    for box in bboxs:
        x1 = clip(box[0], 0, oriw) / factorx       # left
        y1 = clip(box[1], 0, orih) / factory       # top
        x2 = clip(box[2], 0, oriw) / factorx       # right
        y2 = clip(box[3], 0, orih) / factory       # bottom
        center = [(x1 + x2) / 2, (y1 + y2) / 2]             # 框的中心作为center点
        ...
        sigmax = (x2-x1) / (6*alpha) + 0.8               # sigma根据框的大小得到，表示框的大小的系数
        sigmay = (y2-y1) / (6*alpha) + 0.8
        # print (sigmax, sigmay)
        wh = [x2 - x1, y2 - y1]                          # wh 为框的宽高
        # sigmas.append(sigma)
...
def prepare_kps(kps, orih, oriw, outh, outw):
    ...
    for kp in kps:
        ...
        keypoints.append(kp)
        sigmas.append(2.5)
    return keypoints,sigmas                 # 转换为输出大小后keypoints点位置，sigma均为2.5
...
def read_cpn_json(json_file):      # 从单人估计label文件读取label
    '''
    read ai-format json files, returns:
    1. img_ids, a list, contains all img id
    2. id_bboxs_dict, a dictory, kye is img_id, value is corresponding bboxes as format [ [x1, y1, x2, y2], ..., [x1, y1, x2, y2] ]
    ...
    annos = json.load(f)
    for anno in annos:
        img_id = anno['imgInfo']['img_paths']
        if len(img_ids) != 0:
            if img_ids[-1] != img_id:
                img_ids.append(img_id)
                bboxs = []
                bboxs.append(anno['unit']['GT_bbox'])      # 左上右下
                id_bboxs_dict[img_id] = bboxs
                kps = []
                kps.append(anno['unit']['keypoints'])
                id_kps_dict[img_id] = kps
            else:
                id_bboxs_dict[img_id].append(anno['unit']['GT_bbox'])
                id_kps_dict[img_id].append(anno['unit']['keypoints'])
        else:
            img_ids.append(img_id)
            bboxs = []
            bboxs.append(anno['unit']['GT_bbox'])  # 左上右下
